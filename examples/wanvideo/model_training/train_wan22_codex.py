...
class WanTrainingModule(DiffusionTrainingModule):
    def __init__(
        ...
        if created_vace and not wan22_vace_random_init:
            # 从Wan2.2 TI2V 5B 的DiT权重初始化Wan2.2 Codex VACE模块
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="One or more base directories for AgiBot/Arxx5 multiview VACE training.",
    )
    parser.add_argument(
        "--agibot_multiview_val_base_path",
        type=str,
        default=None,
        help="Validation base directory for Arxx5 multiview training. Defaults to the first train base path.",
    )
    parser.add_argument(
        "--agibot_multiview_val_tail_episodes",
        type=int,
        default=0,
        help="Use the last N episodes from the validation base path as validation, excluding them from training if that base is also in train paths.",
    )
    ...
    if args.dataset == "AgisubDataset":
        ...
        dataset = AgiBotWCDataset4WanControlmultiview(
            base_path=os.path.join(args.agibot_multiview_base_path[0], "train"),
            num_frames=args.num_frames,
            context_length=1,
            repeat=args.dataset_repeat,
            dataset_type="vace",
        ...
        val_dataset = AgiBotWCDataset4WanControlmultiview(
            base_path=os.path.join(args.agibot_multiview_base_path[0], "val"),
            num_frames=args.num_frames,
            context_length=1,
            repeat=args.dataset_repeat,
            dataset_type="vace",
        ...
        for base_path in args.agibot_multiview_base_path:
            ...
            train_datasets.append(
                Arxx5Dataset4Wancontrolmultiview(
                    base_path=base_path,
                    camera_sample_mode=args.dataset_camera_sample_mode,
