...
def overlay_control_on_rgb(rgb, control, alpha=0.70):
    rgb = np.asarray(rgb, dtype=np.uint8)
    control = np.asarray(control, dtype=np.uint8)
    if rgb.shape != control.shape:
        control = np.array(Image.fromarray(control).resize((rgb.shape[1], rgb.shape[0]), Image.BILINEAR))
    bg = np.array([50, 50, 50], dtype=np.int16)
    mask = np.any(np.abs(control.astype(np.int16) - bg[None, None, :]) > 8, axis=-1)
    out = rgb.astype(np.float32)
    out[mask] = (1.0 - alpha) * out[mask] + alpha * control.astype(np.float32)[mask]
    return np.clip(out, 0, 255).astype(np.uint8)


def as_uint8_list(frames):
    return [np.asarray(frame, dtype=np.uint8) for frame in (frames or [])]


def save_rollout_videos(generated_history, control_history, ray_o_history, ray_d_history, robot_history):
    out_dir = os.path.join(tempfile.gettempdir(), "wan22_arxx5_action14_rollout")
    os.makedirs(out_dir, exist_ok=True)
    token = uuid.uuid4().hex[:10]

    generated_history = as_uint8_list(generated_history)
    control_history = as_uint8_list(control_history)
    ray_o_history = as_uint8_list(ray_o_history)
    ray_d_history = as_uint8_list(ray_d_history)
    robot_history = as_uint8_list(robot_history)

    overlay_frames = [
        overlay_control_on_rgb(rgb, control)
        for rgb, control in zip(generated_history, control_history)
    ]

    overlay_path = os.path.join(out_dir, f"{token}_generated_traj_overlay_full.mp4")
    ray_o_path = os.path.join(out_dir, f"{token}_ray_o_full.mp4")
    ray_d_path = os.path.join(out_dir, f"{token}_ray_d_full.mp4")
    robot_path = os.path.join(out_dir, f"{token}_arxx5_fk_full.mp4")

    save_frames_to_mp4(overlay_frames, overlay_path, APP_ARGS.fps)
    save_frames_to_mp4(ray_o_history, ray_o_path, APP_ARGS.fps)
    save_frames_to_mp4(ray_d_history, ray_d_path, APP_ARGS.fps)
    save_frames_to_mp4(robot_history, robot_path, APP_ARGS.fps)
    return overlay_path, ray_o_path, ray_d_path, robot_path
...
def load_init_episode(episode_key):
    ...
    control0 = np.array(control_video[0])
    ray_o0 = np.array(ray_map_o[0])
    ray_d0 = np.array(ray_map_d[0])
    robot0 = np.array(robot_preview)
    return (
        action_text,
        None,
        None,
...
def run_generation(
    ...
    raw_hw_state,
    step_state,
    generated_history_state,
    control_history_state,
    ray_o_history_state,
    ray_d_history_state,
    robot_history_state,
):
    ...
    generated_history = as_uint8_list(generated_history_state) + [np.array(frame) for frame in generated_future]
    control_history = as_uint8_list(control_history_state) + [np.array(frame) for frame in control_future]
    ray_o_history = as_uint8_list(ray_o_history_state) + [np.array(frame) for frame in ray_o_future]
    ray_d_history = as_uint8_list(ray_d_history_state) + [np.array(frame) for frame in ray_d_future]
    robot_frame = np.array(robot_preview)
    robot_history = as_uint8_list(robot_history_state) + [robot_frame for _ in generated_future]

    overlay_path, ray_o_path, ray_d_path, robot_path = save_rollout_videos(
        generated_history,
        control_history,
        ray_o_history,
        ray_d_history,
    ...
    status = (
        ...
        f"rollout_video_frames={len(generated_history)}\n"
...
def build_demo():
    ...
    with gr.Blocks(title="Wan2.2 ARXX5 Online Inference") as demo:
        ...
        with gr.Row():
            overlay_video = gr.Video(label="Generated Three-View + Traj Overlay (full rollout)")
            ray_o_video = gr.Video(label="Ray Map O (full rollout)")
            ray_d_video = gr.Video(label="Ray Map D (full rollout)")
            robot_video = gr.Video(label="ARXX5 FK (full rollout)")

        status = gr.Textbox(label="Status", lines=8)
        context_state = gr.State()
        raw_hw_state = gr.State()
        step_state = gr.State(0)
        generated_history_state = gr.State()
        control_history_state = gr.State()
        ray_o_history_state = gr.State()
        ray_d_history_state = gr.State()
        robot_history_state = gr.State()
