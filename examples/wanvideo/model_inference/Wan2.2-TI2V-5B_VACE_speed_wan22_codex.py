...
def run_episode(pipe, ep_info, args):
    ...
    for chunk_idx in range(num_chunks):
        ...
        timings.append((elapsed, valid_prediction_count))
        print(
            f"[SPEED] episode={ep_info['episode_key']} chunk={chunk_idx + 1}/{num_chunks} "
            f"steps={args.num_inference_steps} elapsed_sec={elapsed:.3f} "
            f"pred_frames={valid_prediction_count} fps={valid_prediction_count / elapsed:.3f}"
        )
        ...
        if args.benchmark_chunks is not None and len(timings) >= args.benchmark_chunks:
            break

    if args.benchmark_chunks is None and not args.dry_run and len(generated_frames) != total_frames:
        raise RuntimeError(
...
def main():
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default="/mnt/data/zsq/DiffSynth-Studio/outputs/speed_wan22_vace_epoch29_codex",
    )
    ...
    parser.add_argument("--episode_limit", type=int, default=1)
    parser.add_argument("--benchmark_chunks", type=int, default=5)
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=[
            "/mnt/data/zsq/DiffSynth-Studio/outputs/Wan2.2-TI2V-5B-VACE-wan22-codex-ray-action-perspective-continue20/epoch-29-merge.safetensors",
    ...
    for ep in episodes:
        ...
        all_timings.extend(run_episode(pipe, ep_info, args))

    if all_timings:
        total_sec = sum(elapsed for elapsed, _ in all_timings)
        total_frames = sum(frames for _, frames in all_timings)
        print(
            f"[SUMMARY] chunks={len(all_timings)} steps={args.num_inference_steps} "
            f"total_sec={total_sec:.3f} avg_sec_per_chunk={total_sec / len(all_timings):.3f} "
            f"pred_frames={total_frames} pred_fps={total_frames / total_sec:.3f}"
        )
    print(f"[DONE] Speed test output_dir={args.output_dir}")
