...
- traj_map: FK(action) per-camera actions + FK(action) per-camera w2c
...
from diffsynth.trainers.arxx5_urdf_fk import configure as configure_fk
...
def freeze_selected_arms(action_ds, freeze_after_step, freeze_arm):
    if freeze_after_step < 0:
        return action_ds

    if freeze_after_step >= len(action_ds):
        raise ValueError(
            f"--freeze_after_step={freeze_after_step} out of range for downsampled length {len(action_ds)}"
        )

    action_ds = action_ds.copy()

    if freeze_arm in {"left", "both"}:
        action_ds[freeze_after_step:, 0:7] = action_ds[freeze_after_step, 0:7]

    if freeze_arm in {"right", "both"}:
        action_ds[freeze_after_step:, 7:14] = action_ds[freeze_after_step, 7:14]
...
def build_episode_info(ep, args):
    ...
    action_ds = freeze_selected_arms(action_ds, args.freeze_after_step, args.freeze_arm)
    ...
    traj_actions_by_cam, traj_w2c_by_cam = make_per_camera_sequences(
        fk_ee_pose_ds, action_ds, extr, args.camera_axis_mode
    )
    ray_w2c_by_cam = make_center_ray_w2c_sequences(fk_ee_pose_ds, extr, args.camera_axis_mode)
...
def main():
    ...
    parser.add_argument("--agx_base", type=str, default="/mnt/workspace/zsq/arx_policy_rollout_new_continue")
    ...
    parser.add_argument("--predict_frames", type=int, default=16)
    parser.add_argument(
        "--freeze_after_step",
        type=int,
        default=-1,
        help="Freeze action from this downsampled timestep onward; -1 disables freezing.",
    )
    parser.add_argument(
        "--freeze_arm",
        type=str,
        ...
        help="Which arm to freeze from --freeze_after_step onward.",
    )
    ...
    parser.add_argument(
        "--vace_model_path",
        type=str,
        default="/mnt/workspace/zsq/DiffSynth-Studio/outputs/Wan2.2-TI2V-5B-VACE-wan22-codex-ray-action-perspective-arxx5-rolloutdatasetnew/epoch-99.safetensors",
    ...
    print(f"[INFO] traj_source={ep_info['traj_source']}")
    if args.freeze_after_step >= 0:
        freeze_raw_idx = int(ep_info["ds_indices"][args.freeze_after_step])
        print(
            f"[INFO] freeze_after_step={args.freeze_after_step} freeze_arm={args.freeze_arm} "
            f"freeze_raw_idx={freeze_raw_idx} downsampled_length={ep_info['T_ds']}"
        )
