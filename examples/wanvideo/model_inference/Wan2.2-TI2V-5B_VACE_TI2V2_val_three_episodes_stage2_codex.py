...
def get_chunk_by_ids_with_pad(frames, frame_ids):
...
def load_pipe(model_paths, device):
    ...
    class WanVideoUnit_TI2V2ContextEmbedder(PipelineUnit):
        ...
        def process(self, pipe, input_video, noise, latents, tiled, tile_size, tile_stride):
            ...
            if len(input_video) < 5:
                raise ValueError(f"TI2V2 context inference expects 5 context frames, got {len(input_video)}.")
            ...
            if latents is None:
                latents = noise.clone()
            latents[:, :, 0:1] = context_latents[:, :, 0:1]
            low_noise_timestep = pipe.scheduler.timesteps[-1].reshape(1).to(dtype=pipe.torch_dtype, device=pipe.device)
            latents[:, :, 1:2] = pipe.scheduler.add_noise(
                context_latents[:, :, 1:2],
                noise[:, :, 1:2],
                low_noise_timestep,
            )
            return {"latents": latents, "first_frame_latents": context_latents[:, :, 0:1]}
    ...
    pipe.units = [
        WanVideoUnit_TI2V2ContextEmbedder() if unit.__class__.__name__ == "WanVideoUnit_InputVideoEmbedder" else unit
...
def run_episode(pipe, ep_info, args):
    ...
    for chunk_idx in range(num_chunks):
        horizon_start_idx = 1 + chunk_idx * predict_frames
        valid_prediction_count = min(predict_frames, total_frames - horizon_start_idx)
        if chunk_idx == 0:
        ...
        print(
            ...
            f"horizon_start_idx={horizon_start_idx} "
        ...
        print(
            f"[SPEED] episode={ep_info['episode_key']} chunk={chunk_idx + 1}/{num_chunks} "
            f"elapsed_sec={elapsed:.3f} pred_frames={valid_prediction_count} "
            f"fps={valid_prediction_count / elapsed:.3f}"
        )
        new_frames = generated[args.context_frames: args.context_frames + valid_prediction_count]
        for frame in new_frames:
            if not isinstance(frame, Image.Image):
                frame = Image.fromarray(np.array(frame))
            generated_frames.append(frame)

        if args.max_chunks is not None and (chunk_idx + 1) >= args.max_chunks:
            break

    if args.max_chunks is None and not args.dry_run and len(generated_frames) != total_frames:
        raise RuntimeError(
...
def main():
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default="/mnt/workspace/zsq/Agi2024subset_split/val_wan22_vace_ti2v2_stage2_epoch49_codex",
    )
    ...
    parser.add_argument("--parallel_episodes", action="store_true")
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=[
            "/mnt/data/zsq/DiffSynth-Studio/outputs/Wan2.2-TI2V-5B-TI2V2-context5-horizon8-codex/epoch-39.safetensors",
            "/mnt/workspace/zsq/DiffSynth-Studio/outputs/Wan2.2-TI2V-5B-VACE-TI2V2-context5-horizon8-codex-stage2-epoch39/epoch-89.safetensors",
    ...
    if args.predict_frames != args.horizon_frames:
    ...
    if args.parallel_episodes and not args.worker:
        gpu_ids = [gpu.strip() for gpu in args.gpus.split(",") if gpu.strip()]
        if len(gpu_ids) < len(episodes):
            raise RuntimeError(f"Need at least {len(episodes)} GPU ids, got {gpu_ids}.")
        logs_dir = os.path.join(args.output_dir, "logs")
        os.makedirs(logs_dir, exist_ok=True)
        procs = []
        for ep, gpu_id in zip(episodes, gpu_ids):
            cmd = [
                sys.executable,
                __file__,
                "--worker",
            ...
            print(f"[LAUNCH] episode={ep['episode_key']} gpu={gpu_id} log={log_path}")
            with open(log_path, "w") as log_f:
                procs.append((ep["episode_key"], gpu_id, log_path, subprocess.Popen(cmd, env=env, stdout=log_f, stderr=subprocess.STDOUT)))
        failed = []
        for ep_key, gpu_id, log_path, proc in procs:
            code = proc.wait()
            print(f"[DONE_WORKER] episode={ep_key} gpu={gpu_id} returncode={code} log={log_path}")
            if code != 0:
                failed.append((ep_key, gpu_id, log_path, code))
        if failed:
            raise RuntimeError(f"Some workers failed: {failed}")
        print(f"[DONE] Parallel results saved to {args.output_dir}")
        return
