...
def parse_args():
    ...
    p.add_argument("--fk-urdf-path", type=str, default=None)
    p.add_argument("--fk-offset-path", type=str, default=None)
    p.add_argument("--fk-state-unit", type=str, default="rad", choices=["rad", "deg"])
    return p.parse_args()
...
def main():
    ...
    dataset = Arxx5Dataset4Wancontrolmultiview(
        ...
        fk_urdf_path=args.fk_urdf_path,
        fk_offset_path=args.fk_offset_path,
        fk_state_unit=args.fk_state_unit,
    )
    ...
    for i in range(len(video)):
        cols = [
            ...
            add_title(control[i], "control_video / fk_action_traj_map"),
