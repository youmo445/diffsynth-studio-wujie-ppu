...
ROBOTWIN_GRIPPER_BIAS = 0.12
ROBOTWIN_GRIPPER_BIAS_AXIS = "x"
ROBOTWIN_SCHEME_NAME = "scheme1_wrist_own_center_true_other"
...
def gripper_bias_vector(bias=ROBOTWIN_GRIPPER_BIAS, axis=ROBOTWIN_GRIPPER_BIAS_AXIS):
    ...
    vec[axis_to_idx[name]] = sign * float(bias)
    return vec
...
def build_robotwin_multiview_condition_videos(
    ...
    for cam in camera_names:
        ...
        c2w = w2c_to_c2w(w2c)
        wrist_center_side = None
        if wrist_center_control:
            if cam == "left_camera":
                wrist_center_side = "left"
            elif cam == "right_camera":
                wrist_center_side = "right"
